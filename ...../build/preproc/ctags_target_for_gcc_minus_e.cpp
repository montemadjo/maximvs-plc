# 1 "/home/zoran/A/evirad sistemi/WT32/maximvs-plc/Boards/simple/simple.ino"
/*
...
# 27 "/home/zoran/A/evirad sistemi/WT32/maximvs-plc/Boards/simple/simple.ino" 2
# 28 "/home/zoran/A/evirad sistemi/WT32/maximvs-plc/Boards/simple/simple.ino" 2
# 29 "/home/zoran/A/evirad sistemi/WT32/maximvs-plc/Boards/simple/simple.ino" 2
...
void __attribute__((section(".iram1" "." "16"))) isr_WG1_D0() {
  if(digitalRead(35 /* J25*/) || digitalRead(35 /* J25*/)) return; // filter koji eliminiše kratke smetnje
  card1 <<=1;
  pulseTime1=millis();
}

void __attribute__((section(".iram1" "." "17"))) isr_WG1_D1() {
  if(digitalRead(14 /* J24*/) || digitalRead(14 /* J24*/)) return;
  card1 <<= 1; ++card1;
  pulseTime1=millis();
}

void __attribute__((section(".iram1" "." "18"))) isr_WG2_D0() {
  if(digitalRead(36 /* J19*/) || digitalRead(36 /* J19*/)) return;
  card2 <<=1;
  pulseTime2=millis();
}
void __attribute__((section(".iram1" "." "19"))) isr_WG2_D1() {
  if(digitalRead(39 /* J18*/) || digitalRead(39 /* J18*/)) return;
  card2 <<=1; ++card2;
  pulseTime2=millis();
}
...
void WiFiEvent(system_event_id_t event) // strange WiFiEvent? we are wired?
{
    switch (event)
    {
...
void setup()
{
    ...
    ETH.begin(1, 16, 23, 18, ETH_PHY_LAN8720, ETH_CLOCK_GPIO17_OUT); // Enable ETH
    ...
    pinMode(35 /* J25*/, 0x01);
    pinMode(14 /* J24*/, 0x01);
    pinMode(12 /* J26*/, 0x02); digitalWrite(12 /* J26*/, 0x1); // nula je aktivna
    pinMode(36 /* J19*/, 0x01);
    pinMode(39 /* J18*/, 0x01);
    pinMode(15 /* J20*/, 0x02); digitalWrite(15 /* J20*/, 0x1);

    pinMode(2 /*J31*/, 0x02); digitalWrite(2 /*J31*/, 0x0); // jedinica je aktivna
    pinMode(4 /*J30*/, 0x02); digitalWrite(4 /*J30*/, 0x0);

    // Attach interrupt for all 4 inputs on the board.
    attachInterrupt(35 /* J25*/, isr_WG1_D0, 0x02);
    attachInterrupt(14 /* J24*/, isr_WG1_D1, 0x02);
    attachInterrupt(36 /* J19*/, isr_WG2_D0, 0x02);
    attachInterrupt(39 /* J18*/, isr_WG2_D1, 0x02);
}
...
void loop()
{
    ...
    if(card1 && pulseTime1 && millis() > pulseTime1 + 10 ) {
      Serial.print("CARD1="); Serial.print(card1, 16); Serial.println(""); card1=0;
    }
    if(card2 && pulseTime2 && millis() > pulseTime2 + 10 ) {
      Serial.print("CARD2="); Serial.print(card2, 16); Serial.println(""); card2=0;
    }
}
